(()=>{
  ...
  function periodHtml(data,label){return `<div class="period-main"><div><small>${label}</small><strong>${data.beer} Hefeweizen</strong></div><strong>${euro(data.cost)}</strong></div><div class="period-split"><span>${data.paid} bezahlt</span><span>${data.free} kostenlos</span></div>${breakdownHtml(data)}`}
