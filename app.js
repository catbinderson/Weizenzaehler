(()=>{
  ...
  function normalize(){
    ...
    state.days=state.days.map(r=>({id:String(r.id||`${Date.now()}-${Math.random().toString(36).slice(2,8)}`),date:String(r.date||''),count:Number(r.count||0),unitPrice:Number(r.unitPrice||0),total:Number(r.total||0),closedAt:String(r.closedAt||'')})).filter(r=>/^\d{4}-\d{2}-\d{2}$/.test(r.date)&&Number.isFinite(r.count)&&r.count>=0&&Number.isFinite(r.unitPrice)&&r.unitPrice>0&&Number.isFinite(r.total)&&r.total>=0);
    if(!state.current||state.current.date!==today())state.current={date:today(),count:0};
    state.current.count=Math.max(0,Math.floor(Number(state.current.count)||0));
  }
  ...
  function aggregate(rows){const beer=rows.reduce((s,r)=>s+Number(r.count||0),0),cost=rows.reduce((s,r)=>s+Number(r.total||0),0),by=new Map();for(const r of rows){const p=Number(r.unitPrice||0),k=p.toFixed(2);if(!by.has(k))by.set(k,{price:p,count:0,total:0});const b=by.get(k);b.count+=Number(r.count||0);b.total+=Number(r.total||0)}return {beer,cost,breakdown:[...by.values()].sort((a,b)=>a.price-b.price)}}
  ...
  function render(){normalize();save();const p=priceFor(state.current.date);$('#todayDate').textContent=fmtDate(state.current.date);$('#count').textContent=state.current.count;$('#runningTotal').textContent=euro(totalForCurrent());$('#currentPrice').textContent=`Aktueller Preis: ${euro(p)}`;$('#minusBtn').disabled=state.current.count<=0;$('#closeDayBtn').disabled=state.current.count<=0;const now=new Date(),todayEnd=new Date();todayEnd.setHours(23,59,59,999);renderStat('wtd',aggregate(state.days.filter(r=>inRange(r,mondayOf(now),todayEnd))));renderStat('mtd',aggregate(state.days.filter(r=>inRange(r,monthStart(now),todayEnd))));renderStat('ytd',aggregate(state.days.filter(r=>inRange(r,yearStart(now),todayEnd))));const w=$('#weekSelect').value||isoWeekString(now);$('#weekSelect').value=w;const [ws,we]=weekRange(w);$('#weekResult').innerHTML=periodHtml(aggregate(state.days.filter(r=>inRange(r,ws,we))),`KW ${Number(w.split('W')[1])}`);const mv=$('#monthSelect').value||`${now.getFullYear()}-${pad(now.getMonth()+1)}`;$('#monthSelect').value=mv;const [ms,me]=monthRange(mv);$('#monthResult').innerHTML=periodHtml(aggregate(state.days.filter(r=>inRange(r,ms,me))),new Intl.DateTimeFormat('de-DE',{month:'long',year:'numeric'}).format(ms));const history=[...state.days].sort((a,b)=>(b.closedAt||b.date).localeCompare(a.closedAt||a.date));$('#history').innerHTML=history.length?history.map(r=>`<div class="history-row"><div><strong>${fmtDate(r.date)}</strong><small>${r.count} Bier · ${euro(r.unitPrice)} pro Bier</small></div><div class="amount">${euro(r.total)}</div></div>`).join(''):'<div class="history-empty">Noch kein Tagesabschluss gespeichert.</div>';$('#priceHistory').innerHTML=[...state.prices].reverse().map((x,i)=>`<div class="price-row"><span>ab ${new Intl.DateTimeFormat('de-DE').format(new Date(`${x.from}T12:00:00`))}${i===0?' · aktuell':''}</span><b>${euro(x.price)}</b></div>`).join('');$('#priceInput').value=p.toFixed(2);$('#priceFrom').value=today()}
